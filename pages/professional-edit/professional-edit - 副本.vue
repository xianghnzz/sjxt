		...
		<template v-slot:content>
			<view class="" style="height: 100%;">
				...
				<view class="flex align-center justify-center"
					...
					<view @click.stop="claerActive(null)" class="position-relative border border-danger p-4 overflow-hidden" :style="`width: ${dragWidth}px; height: ${dragHeight}px`">
						<!-- 绘制背景 -->
						...
						<df-moveable @click.stop="setItemActive(index)"
							...
							:id="item.id" :active="item.active" :x="item.x" :y="item.y" :w="item.width" :h="item.height"
							:scale="item.scale" :rotate="item.r" @updateItem="updateItem" @removeItem="removeItem" @moveableEnd="moveableEnd">
							<template v-if="item.type == 'photo'" >
								<image v-if="item.url" :style="`border-radius: ${item.radius}px;`"
									style="width: 100%; height: 100%;" :src="utils.showImageUrl(item.url || '')" mode="">
								</image>
								<view v-else class=""
									:style="`border-radius: ${item.radius}px;`"
									style="width: 100%; height: 100%; background-color: rgba(51, 102, 255, 0.2);">
								</view>
							</template>
							<image v-else :style="`border-radius: ${item.radius}px;`"
								style="width: 100%; height: 100%;" :src="utils.showImageUrl(item.url || '')" mode="">
							</image>
						</df-moveable>
					</view>
