	...
	<page bgColor="#fff" topColor="#00AEEF" :topHeight="88" :bottomHeight="180" :refresherEnabled="false">
		<template v-slot:navBar>
			<view class="text-center flex align-center justify-center" style="height: 88rpx;">
				<text class="font-36 text-white">书籍模版编辑</text>
				...
				<view v-if="coverPage.pages_id" class="">
					<view class="flex flex-column align-center justify-center">
						...
						<view @click="setPageInfo(coverPage, 'coverPage')"
							class="position-relative flex align-center justify-center border border-primary" style="width: 344rpx;">
							...
							<view class="position-absolute top-0 left-0 right-0 bottom-0">
								<!-- 循环覆盖物    -->
								<view v-for="item2, index2 in coverPage.pages_content.imgList" :key="index2" class="position-absolute top-0 left-0">
									<view @click.stop="chooseImage('coverPage', index2)" v-if="item2.type == 'photo'"
										...
										<text v-else class="text-primary font-40">+</text>
									</view>
							...
							<view class="position-absolute top-0 left-0 right-0 bottom-0">
								...
								<view v-for="item2, index2 in item.pages_content.imgList" :key="index2" class="">
									<view @click.stop="chooseImage(index, index2)" v-if="item2.type == 'photo'"
										class="position-absolute flex align-center justify-center overflow-hidden"
										style="background-color: rgba(51, 102, 255, 0.2);"
										:style="`transform: scale(${item2.scale}) rotate(${item2.r || 0}deg);border-radius: ${item2.radius}px;top:${item2.y /  item.bili}px;left:${item2.x /  item.bili}px;width: ${item2.width /  item.bili}px; height:${item2.height /  item.bili}px;`">
										<image v-if="item2.url"
