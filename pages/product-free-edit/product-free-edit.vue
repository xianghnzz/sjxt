<template>
	<page bgColor="#fff" topColor="#00AEEF" :topHeight="88" :bottomHeight="240" :refresherEnabled="false">
		...
		<template v-slot:content>
			<view class="px-1 py-2 flex flex-wrap">
				<view v-for="(item, index) in product_print_list" :key="index" class="m-1" style="width: 344rpx">
					<!-- 页面区域 -->
					<view @click="setPageInfo(item, index)" class="position-relative flex align-center justify-center" style="width: 344rpx">
						<image class="border" v-if="item?.pages_content?.bgImg" style="width: 344rpx" :src="utils.showImageUrl(item.pages_content.bgImg)" mode="widthFix"></image>
						...
						<view class="position-absolute top-0 left-0 right-0 bottom-0">
							...
							<view v-for="(item2, index2) in item.pages_content.imgList" :key="index2" class="">
								...
								<view v-else class="position-absolute">
									<image class="position-absolute" :style="`transform:scale(${item2.scale}) rotate(${item.r || 0}deg);border-radius: ${item.radius}px;top:${item2.y / item.bili}px;left:${item2.x / item.bili}px;width: ${item2.width / item.bili}px; height:${item2.height / item.bili}px;`" :src="utils.showImageUrl(item2.url)" mode=""></image>
								</view>
							</view>
						</view>
					...
					<view class="flex align-center justify-center py-2 font-30 font-weight-bold">
						<image @click="removeItem(index)" style="width: 50rpx; height: 50rpx" src="/static/images/common/reduce.png" mode=""></image>
						...
						<image @click="addItem(item)" style="width: 50rpx; height: 50rpx" src="/static/images/common/add.png" mode=""></image>
					</view>
				</view>
			</view>
			<!-- 确认定制弹窗 -->
		...
		<template v-slot:bottomBar>
			<view class="flex align-center justify-center text-ellipsis px-1" :style="`background-color:${configsInfo2.configs_content}; color:${configsInfo2.configs_illustrate};font-size: ${configsInfo2.configs_use || 28}rpx;`" style="width: 750rpx; height: 60rpx">
			...
			<view class="bg-white flex align-center justify-center px-2" style="height: 120rpx">
				...
				<view v-if="isOk" @click="showModal = 1" class="flex-1 flex align-center justify-center linear-gradient rounded-circle ml-1" style="height: 90rpx">
					<text class="text-white">制作完成</text>
				</view>
				<view v-else @click="showModal = 1" class="flex-1 flex align-center justify-center bg-info rounded-circle ml-1" style="height: 90rpx">
...
const { triggered, product_print_list, bili, showModal, configsInfo, configsInfo2, cItem, cIndex } = toRefs(data);
	...
	uni.chooseImage({
		...
		success: async (res) => {
			for (let i = 0; i < res.tempFilePaths.length; i++) {
				...
				console.log('res2.data.url', res2.data.url);
			}
		}
	});
};
