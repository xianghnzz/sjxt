<template>
	<page bgColor="#fff" topColor="#00AEEF" :topHeight="88" :bottomHeight="240" :refresherEnabled="false">
		<template v-slot:navBar>
			<view class="text-center flex align-center justify-center" style="height: 88rpx;">
				<text class="font-36 text-white">无限模式商品模版编辑</text>
			...
			<view class="py-2">
				<view v-if="coverPage.pages_id" class="">
					<view class="flex flex-column align-center justify-center">
						<!-- 页面区域 -->
						<view @click="setPageInfo(coverPage, 'coverPage')"
							class="position-relative flex align-center justify-center" style="width: 344rpx;">
							<image class="border" v-if="coverPage?.pages_content?.bgImg" style="width: 344rpx;"
								:src="utils.showImageUrl(coverPage.pages_content.bgImg)" mode="widthFix"></image>
							<!-- 覆盖区域 -->
							<view class="position-absolute top-0 left-0 right-0 bottom-0">
								<!-- 循环覆盖物    -->
								<view v-for="item2, index2 in coverPage.pages_content.imgList" :key="index2" class="">
									<view @click.stop="chooseImage('coverPage', index2)" v-if="item2.type == 'photo'"
										class="position-absolute flex align-center justify-center overflow-hidden"
										style="background-color: rgba(51, 102, 255, 0.2);"
										:style="`transform:scale(${item2.scale}) rotate(${item2.r || 0}deg);border-radius: ${item2.radius}px;top:${item2.y / coverPage.bili}px;left:${item2.x / coverPage.bili}px;width: ${item2.width / coverPage.bili}px; height:${item2.height / coverPage.bili}px;`">
										<image v-if="item2.url"
											:style="`width: ${item2.width / coverPage.bili}px; height:${item2.height / coverPage.bili}px;`"
				...
				<view class="px-1 py-2 flex flex-wrap">
					<view v-for="item,index in product_print_list" :key="index" class="m-1" style="width: 344rpx;">
						
						<!-- 页面区域 -->
						<view @click="setPageInfo(item, index)"
							class="position-relative flex align-center justify-center" style="width: 344rpx;">
							<image class="border" v-if="item?.pages_content?.bgImg" style="width: 344rpx;"
								:src="utils.showImageUrl(item.pages_content.bgImg)" mode="widthFix"></image>
								
							<!-- 覆盖区域 -->
							<view class="position-absolute top-0 left-0 right-0 bottom-0">
								<!-- 循环覆盖物    -->
								<view v-for="item2, index2 in item.pages_content.imgList" :key="index2" class="">
									<view @click.stop="chooseImage(index, index2)" v-if="item2.type == 'photo'"
										class="position-absolute border border-primary flex align-center justify-center overflow-hidden"
										style="background-color: rgba(51, 102, 255, 0.2);"
										:style="`transform:scale(${item2.scale}) rotate(${item.r || 0}deg);border-radius: ${item.radius}px;top:${item2.y / item.bili}px;left:${item2.x / item.bili}px;width: ${item2.width / item.bili}px; height:${item2.height / item.bili}px;`">
										<image v-if="item2.url"
											:style="`width: ${item2.width / item.bili}px; height:${item2.height / item.bili}px;`"
											:src="utils.showImageUrl(item2.url)" mode="widthFix"></image>
										<text v-else class="text-primary font-40 font-weight-bold">+</text>
									</view>
									<view v-else class="position-absolute">
						...
						<!-- 页码区域 -->
						<view class="flex align-center justify-center py-2 font-30 font-weight-bold">
							<image @click="removeItem(index)" style="width: 50rpx; height: 50rpx;"
								src="/static/images/common/reduce.png" mode=""></image>
							<text class="ml-2">{{orderFrom?.order_spec_info_obj?.spec_page_prefix}}</text>
							<text class="mx-1">{{index + 1}}</text>
							<text class="mr-2">{{orderFrom?.order_spec_info_obj?.spec_page_suffix}}</text>
							<image @click="addItem(item)" style="width: 50rpx; height: 50rpx;"
								src="/static/images/common/add.png" mode=""></image>
						</view>
					</view>
				</view>
				<view v-if="endPage.pages_id" class="">
					<view class="flex flex-column align-center justify-center">
						<!-- 页面区域 -->
						<view @click="setPageInfo(endPage, 'endPage')"
							class="position-relative flex align-center justify-center" style="width: 344rpx;">
							<image class="border" v-if="endPage?.pages_content?.bgImg" style="width: 344rpx;"
								:src="utils.showImageUrl(endPage.pages_content.bgImg)" mode="widthFix"></image>
							<!-- 覆盖区域 -->
							<view class="position-absolute top-0 left-0 right-0 bottom-0">
								<!-- 循环覆盖物    -->
								<view v-for="item2, index2 in endPage.pages_content.imgList" :key="index2" class="">
									<view @click.stop="chooseImage('endPage', index2)" v-if="item2.type == 'photo'"
										class="position-absolute border border-primary flex align-center justify-center overflow-hidden"
										style="background-color: rgba(51, 102, 255, 0.2);"
										:style="`transform:scale(${item2.scale}) rotate(${item2.r || 0}deg);border-radius: ${item2.radius}px;top:${item2.y / endPage.bili}px;left:${item2.x / endPage.bili}px;width: ${item2.width / endPage.bili}px; height:${item2.height / endPage.bili}px;`">
										<image v-if="item2.url"
											:style="`width: ${item2.width / endPage.bili}px; height:${item2.height / endPage.bili}px;`"
											:src="utils.showImageUrl(item2.url)" mode="widthFix"></image>
										<text v-else class="text-primary font-40 font-weight-bold">+</text>
									</view>
									<view v-else class="position-absolute">
				...
				<view class="rounded-20 bg-white border-box font pb-3" style="width: 600rpx;">
					...
					<view class="py-2 px-4 font-32">
						...
						<view class="flex align-center py-1">
							...
							<view class="flex text-ellipsis">
								<text class="font-weight-bold">{{orderFrom?.order_material_name || '系统提供纸张'}}</text>
							</view>
						</view>
						<view class="flex align-center py-2">
							<image style="width: 24rpx; height: 24rpx;" src="/static/images/common/gg.png"
								mode="widthFix"></image>
					...
					<view class="px-5 flex align-center justify-center px-2">
						<view @click="showModal = 0"
							class="flex-1 flex align-center justify-center bg-info rounded-circle mr-2"
							style="height: 80rpx;">
							<text class="text-white">取消</text>
						</view>
						<view @click="complete"
							class="flex-1 flex align-center justify-center linear-gradient rounded-circle ml-2"
							style="height: 80rpx;">
		...
		<template v-slot:bottomBar>
			<view class="flex align-center justify-center text-ellipsis px-1"
				:style="`background-color:${configsInfo2.configs_content}; color:${configsInfo2.configs_illustrate};font-size: ${configsInfo2.configs_use || 28}rpx;`"
				style="width: 750rpx; height: 60rpx;">
				<text class="text-ellipsis text-center">{{configsInfo2.configs_value}}</text>
			</view>
			<view class="flex align-center justify-center text-ellipsis px-1"
			...
			<view class="bg-white flex align-center justify-center px-2" style="height: 120rpx;">
				<view @click="chooseImageList"
					class="flex-1 flex align-center justify-center bg-warning rounded-circle mr-1"
					style="height: 90rpx;">
					<text class="text-white">上传图片({{havePicture}}/{{pictureNumber}})</text>
				</view>
				<view v-if="isOk" @click="showModal = 1"
					class="flex-1 flex align-center justify-center linear-gradient rounded-circle ml-1"
					style="height: 90rpx;">
					<text class="text-white">制作完成</text>
				</view>
				<view v-else @click="showModal = 1"
					class="flex-1 flex align-center justify-center bg-info rounded-circle ml-1" style="height: 90rpx;">
					<text class="text-white">未达定制倍数</text>
				</view>
			</view>
		</template>
	</page>
...
<script setup>
	...
	// 判断最少张数 和 数量倍数
	const isOk = computed(() => {
		const number = orderFrom.value?.order_spec_info_obj?.spec_number || 0;
		// 数量为零 不限制
		const multiple = orderFrom.value?.order_spec_info_obj?.spec_multiple || 1;
		if (data.product_print_list.length >= 100) {
			return true
		} else if (number == 0) {
			return true
		} else if (data.product_print_list.length >= number && data.product_print_list.length % multiple == 0) {
			return true
		} else {
			return false
		}
	...
	const setPageInfo = (item, index) => {
		...
		// 	// 采用反面模板
		// 	data.cItem.product_type = 2;
		// } else {
		// 	data.cItem.product_type = 1;
		// }
	...
	const updatePageInfo = (item) => {
		...
		if (data.cIndex == 'coverPage') {
			data.coverPage = Object.assign({}, item);
		} else if (data.cIndex == 'endPage') {
			data.endPage = Object.assign({}, item);
		} else {
	...
	const removeItem = (index) => {
		if (data.product_print_list.length == 1) {
			utils.showToast('只剩最后一页了，不能移除了！！！')
			return false
		}
		data.product_print_list.splice(index, 1)
	}

	const addItem = (item) => {
		const info = JSON.parse(JSON.stringify(item));
		info.pages_id = Math.random().toString(16).slice(2);
		data.product_print_list.push(info)
	}
		...
		uni.chooseImage({
			...
			success: (res) => {
				...
				api.uploadFile({
					...
					if (key1 == 'coverPage') {
						data.coverPage.pages_content.imgList[key2].url = res2.data.url
					} else if (key1 == 'endPage') {
						data.endPage.pages_content.imgList[key2].url = res2.data.url
					} else {
						data.product_print_list[key1].pages_content.imgList[key2].url = res2.data
							.url
					}
				})
			}
	...
	const chooseImageList = () => {
		...
		const number = pictureNumber.value - havePicture.value;
	...
	const getConfigsById2 = () => {
		api.getConfigsById({
			configs_id: 101
		}).then(res => {
			data.configsInfo2 = res.data;
		})
	}
	...
	onLoad((info) => {
		...
		if (orderFrom.value?.order_product_info_obj?.product_print) {
			...
			if (data.product_print_list.length > 0) {
				...
				// console.log('pages_area_width', pages_area_width);
				// console.log('pages_area_width / pages_area_width', );
				setPictureNumber()
				data.coverPage = data.product_print_list[0];
				data.endPage = data.product_print_list[data.product_print_list.length - 1];
				data.product_print_list.shift();
				data.product_print_list.pop();
			}
		} else {
			data.product_print_list = []
		}
