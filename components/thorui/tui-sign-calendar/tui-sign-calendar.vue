<template>
	<view>
		...
		<view class="tui-date-header">
			<view class="tui-date text-primary">日</view>
			<view class="tui-date text-color-999">一</view>
			<view class="tui-date text-color-999">二</view>
			<view class="tui-date text-color-999">三</view>
			<view class="tui-date text-color-999">四</view>
			<view class="tui-date text-color-999">五</view>
			<view class="tui-date text-primary">六</view>
		</view>
		<view class="tui-date-content" :style="{ height: isFixed && fixedHeight ? dateHeight * 6 + 'px' : 'auto' }">
			...
			<view class="tui-date"
				...
				<!-- getStatusData(3, index) -->
				<view
					class="tui-date-text position-relative"
					:style="{
						color: getDescText(index, startDate, endDate) ? '#FF6A30' : getColor(index, 1),
					}">
					<view v-if="getColor(index, 1) == '#13BCE3'">今</view>
					<view v-else>{{ index + 1 }}</view>
					<view v-if="getDescText(index, startDate, endDate)" class="position-absolute bottom-0">
						<image style="width: 16rpx; height: 16rpx;" src="/static/images/sign/ok.png" mode=""></image>
					</view>
					<!-- <view  class="tui-custom-desc"
